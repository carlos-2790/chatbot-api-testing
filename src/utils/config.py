...
class Config:
    ...
    # API Configuration
    API_URL = os.getenv("API_URL", "https://magicloops.dev/api/loop/8f561a04-e7e4-46f0-9c10-e2b23554a41e/run")
    ...
    def validate(cls):
        ...
        if not 0.0 <= cls.QUALITY_THRESHOLD <= 1.0:
            raise ValueError(
                f"QUALITY_THRESHOLD must be between 0.0 and 1.0, got {cls.QUALITY_THRESHOLD}"
            )
        ...
        if cls.REQUEST_RETRY_COUNT < 0:
            raise ValueError(
                f"REQUEST_RETRY_COUNT must be non-negative, got {cls.REQUEST_RETRY_COUNT}"
            )

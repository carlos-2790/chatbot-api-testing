...
class ContentValidator:
    ...
    TESTING_KEYWORDS = {
        'test', 'testing', 'assert', 'mock', 'unittest', 'pytest', 'tdd', 'bdd',
        'integration', 'unit', 'e2e', 'end-to-end', 'automated', 'automation',
        'coverage', 'fixture', 'suite', 'case', 'scenario', 'stub', 'spy',
        'continuous integration', 'ci/cd', 'regression', 'smoke'
    }
    
    # Common testing frameworks
    TESTING_FRAMEWORKS = {
        'pytest', 'unittest', 'jest', 'mocha', 'jasmine', 'junit', 'testng',
        'selenium', 'cypress', 'playwright', 'robot framework', 'cucumber',
        'rspec', 'minitest', 'nose', 'doctest'
    }
    ...
    def count_testing_keywords(text: str) -> int:
        ...
        count = sum(1 for keyword in ContentValidator.TESTING_KEYWORDS 
    ...
    def mentions_frameworks(text: str) -> List[str]:
        ...
        """
        text_lower = text.lower()
        mentioned = [fw for fw in ContentValidator.TESTING_FRAMEWORKS 
                    if fw in text_lower]
        return mentioned
    ...
    def calculate_content_score(text: str) -> float:
        ...
        logger.debug(f"Content score: {normalized_score:.2f} (keywords: {keyword_count}, "
